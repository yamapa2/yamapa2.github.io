class HypoTrochoid extends Trochoid {
	...
	_nextStep() {
		...
		let phi_minus_rbyaslip_phi = this.phi - rbyaslip_phi;

		//	Compute the next point on the Oid
		xt = this.apmr * Math.cos(rbyaslip_phi) + this.s * Math.cos(phi_minus_rbyaslip_phi);
		yt = this.apmr * Math.sin(rbyaslip_phi) - this.s * Math.sin(phi_minus_rbyaslip_phi);
